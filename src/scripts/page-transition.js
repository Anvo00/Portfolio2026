...
if (overlay) {
  if (root.dataset.transition !== "logo") {
    ...
    tl.to(fill, { opacity: 1, duration: 0.4, ease: "power2.out" }, "+=0.01");
    tl.to(outline, { opacity: 0, duration: 0.4, ease: "power2.out" }, "<0.5");

    // Scale up + fade the curtain to reveal the page.
    tl.addLabel("out", "+=0.0");
    tl.to(logo, { scale: 32, transformOrigin: "center center", duration: 0.5, ease: "power2.inOut" }, "out");
    tl.to(overlay, { backgroundColor: "rgba(14,14,14,0)", duration: 0.5, ease: "power2.inOut" }, "out");
    tl.to(logo, { autoAlpha: 0, duration: 0.5, ease: "power2.inOut" }, "out");
